...
function Input.keypressed(key)
    local s = state
    print("[Input] keypressed: " .. key .. " inputMode=" .. tostring(s.inputMode))
    ...
    elseif key == "r" then
        s.openInput("Load file:", "", function(text)
            if text ~= "" then
                local content = love.filesystem.read(text)
                if content then
    ...
    elseif key == "tab" then
        if sf >= 1 and sf <= #faces then
            ...
            print("[Input] Tab -> selectedVertex=" .. s.selectedVertex .. " sf=" .. sf .. " verts=" .. #f.vertices)
    ...
    elseif key == "left" then
        if sv >= 1 and sf >= 1 and sf <= #faces then
            ...
            s.setStatus(string.format("Vertex %d -X", sv))
        elseif sf >= 1 and sf <= #faces then
    ...
    elseif key == "up" then
        if sv >= 1 and sf >= 1 and sf <= #faces then
            ...
            s.setStatus(string.format("Vertex %d +Y", sv))
        elseif sf >= 1 and sf <= #faces then
    ...
    elseif key == "q" then
        if sv >= 1 and sf >= 1 and sf <= #faces then
            ...
            s.setStatus(string.format("Vertex %d -Z", sv))
        elseif sf >= 1 and sf <= #faces then
    ...
    elseif key == "6" then
        if sf >= 1 and sf <= #faces then
            Model.rotate(faces[sf], "x", math.pi / 2)
            s.setStatus("Rotated 90 X")
        end
    elseif key == "7" then
        if sf >= 1 and sf <= #faces then
            Model.rotate(faces[sf], "x", -math.pi / 2)
    ...
    elseif key == "8" then
        if sf >= 1 and sf <= #faces then
            Model.rotate(faces[sf], "y", math.pi / 2)
            s.setStatus("Rotated 90 Y")
        end
    elseif key == "9" then
        if sf >= 1 and sf <= #faces then
            Model.rotate(faces[sf], "z", math.pi / 2)
            s.setStatus("Rotated 90 Z")
        end
    end
end
